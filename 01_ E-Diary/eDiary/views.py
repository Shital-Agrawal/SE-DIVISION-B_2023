...
def searchNotes(request):
    if request.method == 'POST':
        ...
        # Query the notes based on search query and time range
        if start_time:
            notes = Notes.objects.filter(noteTitle__icontains=search_query, CreationDate__gte=start_time)
        else:
            notes = Notes.objects.filter(noteTitle__icontains=search_query)

        return render(request, 'searchNotes.html', {'notes': notes})
    else:
